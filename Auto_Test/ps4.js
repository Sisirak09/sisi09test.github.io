...
function setupRW() {
	...
	debug_log("Webkit Complete..Now Run Exploit !!");
...
function toggle_payload(pld){
	if(pld == "binloader"){
		...
		document.getElementById("progress").innerHTML="Running Exploit.. Please wait!!";
		preloadScripts(['jb.js']);
	}else if(pld == "exploit_old"){
		document.getElementById("progress").innerHTML="Running old Exploit.. Please wait!!";
		preloadScripts(['oldjb.js']);
	}else if(pld == "henb"){
		document.getElementById("progress").innerHTML="Loading Henb V2.1.3.. Please wait..";
		preloadScripts(['preloader.js', 'henb.js', 'loader.js']);	
	}else if(pld == "goldhen"){
